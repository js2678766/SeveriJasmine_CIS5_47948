...
{
    //Declare Variables
    string PC_Name; //To store the Player Character's ("PC's") first and last names
            ...
            case '1':
            {
                ...
                if (PC_Init+InitB > En_Init+3) //Player is first 
                {
                    ...
                    while (Damd > 0 && En_HP > 0) //This loop will continue until someone is knocked down to 0 HP or lower
                    {
                        ...
                        int hit = phit(PC_Hit, PCToHit);
            
                        if (hit >= En_AC) //If the player hits, generate and calculate the player's damage
                        {
                            int Dam;
...
int phit(int PC_Hit, int PCToHit)
{
    int hit;
    cout<<"[You rolled: "<<PC_Hit<<" (+"<<PCToHit<<" bonus) = "<<PC_Hit+PCToHit<<"]"<<endl;
    hit = PC_Hit + PCToHit;
